...
from statsmodels.stats.multivariate import (Results, hotelling_1samp,
    mv_mean_conf_int_simult_stat, mv_mean_conf_int_pointwise_stat)
...
def test_mv_mean():
    ...
    res_stata = Results(p_F=1.25062334808e-09,
                        df_r=22,
                        df_m=2,
                        F=59.91609589041116,
    ...
    res_stata = Results(T2=9.698067632850247,
                        df=10,
                        k=2,
                        N=12,
    ...
    res_stata = Results(p_F=.0423949782937231,
                        df_r=10,
                        df_m=2,
                        F=4.408212560386478,
                        T2=9.69806763285025)
    ...
    ci1 = mv_mean_conf_int_pointwise_stat(dw.mean, np.diag(dw.var), len(x[:,1:]), lin_transf=np.eye(4), alpha=0.05)
    ci2 = mv_mean_conf_int_pointwise_stat(dw.mean, dw.cov, len(x[:,1:]), lin_transf=np.eye(4), alpha=0.05)
...
def test_confint_simult():
    ...
    cii = [mv_mean_conf_int_simult_stat(m, cov, nobs, lin_transf=np.eye(3)[i])[:2]
           for i in range(3)]
    ...
    res_constr = mv_mean_conf_int_simult_stat(m, cov, nobs, lin_transf=[0, 1, -1])

    assert_allclose(res_constr[0], 29.56 - 3.12, rtol=1e-3)
    assert_allclose(res_constr[1], 29.56 + 3.12, rtol=1e-3)

    # TODO: this assumes separate constraints, but we want multiplicity correction
    # test if several constraints or transformations work
    # original, flipping sign, multiply by 2
    res_constr2 = mv_mean_conf_int_simult_stat(m, cov, nobs,
                      lin_transf=[[0, 1, -1], [0, -1, 1], [0, 2, -2]])
